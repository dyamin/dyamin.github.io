...
- **MEGA repo polish (#11, pushed to `dyamin/MEGA` main):** BibTeX TODO filled with the published citation, DOI + bioRxiv linked in the intro, press coverage + yamin.dev in References. **`dyamin/MEGA-Experiment`:** README (what it does, file map, how to run, citation) + MIT license added. Still Daniel-only there: repo *About* descriptions and topics (`eye-tracking`, `episodic-memory`, `machine-learning`, `computational-neuroscience`) — no API access from agent sessions.
