...
**This quarter — the compounding loop** 10. **Write monthly.** The reference class (simonwillison.net, eugeneyan.com, hamel.dev) is blog-centric. Next two essays: "Lessons from shipping LLM agents in clinical genomics" and "No-report evals: testing agent memory without asking the model." 11. **Distribution loop per post:** yamin.dev first (canonical) → LinkedIn summary next day → the strongest 2–3 per year to Hacker News. 12. **One public AI showcase repo:** a small, well-documented agentic-workflow or LLM-eval harness in your domain (literature mining) — the single biggest gap between your LinkedIn claim and public evidence. 13. **One talk:** the MEGA→agent-memory story is a natural meetup/podcast pitch; the Talks & Media page now has a speaking CTA.
