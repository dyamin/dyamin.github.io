...
- **Deployed to master the same day** and verified live (post 200, homepage shows Sep 2026 stamp, news item, Blog nav).
- **MEGA repo polish (#11, pushed to `dyamin/MEGA` main):** BibTeX TODO filled with the published citation, DOI + bioRxiv linked in the intro, press coverage + yamin.dev in References. **`dyamin/MEGA-Experiment`:** README (what it does, file map, how to run, citation) + MIT license added. Still Daniel-only there: repo _About_ descriptions and topics (`eye-tracking`, `episodic-memory`, `machine-learning`, `computational-neuroscience`) — no API access from agent sessions.
- Tried and blocked from an agent session: creating `dyamin/dyamin` (GitHub integration returns 403 on repo creation), the Pages "Enforce HTTPS" setting (Pages API not reachable through the proxy), and the MEGA/MEGA-Experiment About/topics fields (no MCP tool; direct REST is proxy-blocked).
